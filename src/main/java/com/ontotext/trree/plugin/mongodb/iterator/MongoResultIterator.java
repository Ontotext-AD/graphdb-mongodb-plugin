...
import static com.apicatalog.jsonld.lang.Keywords.BASE;
...
import com.ontotext.trree.plugin.mongodb.request.RequestCache;
...
public class MongoResultIterator extends StatementIterator {
  ...
  public MongoResultIterator(
    ...
    this.jsonLdParserConfig.set(GraphDBJSONLDSettings.DOCUMENT_LOADER, documentLoader);
  }
  ...
  protected boolean initialize() {
    if (query == null && aggregation == null) {
      throw new PluginException("There is no search query for Mongo. Please use either of the predicates: "
          + List.of(MongoDBPlugin.QUERY.toString(), MongoDBPlugin.AGGREGATION.toString()));
    }
    ...
    try {
      ...
      throw new PluginException(
          "Could not connect to MongoDB. Please make sure you are using correct authentication. " + ex.getMessage());
    }
  ...
  protected long readNextDocument(Consumer<Model> dataAccumulator) {
      ...
      if (item != null) {
        if (item instanceof List) {
          ...
          if (!listItem.isEmpty() && listItem.get(0) instanceof Document) {
            ...
            if (listItem.size() > 1) {
              plugin.getLogger()
                  .warn("Multiple graphs in mongo document. Selecting the first one for entity:	" + entity);
            }
          } else {
    ...
    try {
      ...
      currentRDF = Rio.parse(reader, docBase, RDFFormat.JSONLD, jsonLdParserConfig, SimpleValueFactory.getInstance(),
          new ParseErrorLogger());

      Resource v = null;
      if (entity != null) {
        try {
          v = VF.createIRI(entity);
        } catch (IllegalArgumentException e) {
          Object contextValue = doc.get(CONTEXT);
          String base = resolveDocumentBase(contextValue);
          if (base != null) {
            try {
              v = VF.createIRI(base, entity);
      ...
      if (customNode instanceof Document) {
        for (Map.Entry<String, Object> val : ((Document) customNode).entrySet()) {
          currentRDF.add(v, VF.createIRI(MongoDBPlugin.NAMESPACE_INST, val.getKey()),
              VF.createLiteral(val.getValue().toString()));
        }
      }
    ...
    if (context instanceof Map) {
      ...
      try {
        JsonStructure jsonStructure =
            ((JsonDocument) documentLoader.loadDocument(URI.create(context.toString()), new DocumentLoaderOptions()))
                .getJsonContent().orElse(null);
      ...
      for (Object ctxItem : (Collection<?>) context) {
        if (ctxItem instanceof Map) {
          ...
          plugin.getLogger().warn("Unsupported @context type. Expected document or remote URI, got : {}", ctxItem);
        }
      }
  ...
  private Object removeExtraQuotes(Object value) {
    if (value instanceof JsonString) {
      ...
      return map.entrySet().stream().collect(Collectors.toMap(Map.Entry::getKey, e -> removeExtraQuotes(e.getValue())));
    } else if (value instanceof List) {
      List<Object> list = (List<Object>) value;
      return list.stream().map(v -> removeExtraQuotes(v)).collect(Collectors.toList());
    }
    return value;
  }
  ...
  private Collation createCollation(String collationString) {
    ...
    if (doc.containsKey("alternate")) {
      builder.collationAlternate(CollationAlternate.fromString(doc.getString("alternate")));
    }

    if (doc.containsKey("maxVariable")) {
      builder.collationMaxVariable(CollationMaxVariable.fromString(doc.getString("maxVariable")));
    }
