...
/**
 * Convenient test case for the scenario: upload date to mongo, query it, verify the result. If you are using the
 * verifyOrderedResult() method you can use the LEARN_MODE option which will generate the output files for you.
 */
public abstract class AbstractMongoBasicTest extends AbstractMongoTest {

  private static final String CONNECT_MONGODB_LOCALHOST = """
  ...
  private static final String DROP_MONGODB_LOCALHOST = """
      PREFIX : <http://www.ontotext.com/connectors/mongodb#>
      PREFIX inst: <http://www.ontotext.com/connectors/mongodb/instance#>
  ...
  protected void verifyResultsCount(String query, int expectedResultsCount) {
    try (RepositoryConnection conn = getRepository().getConnection()) {
      ...
      assertEquals("The results count differs", expectedResultsCount, result.stream().count());
    }
  }
   ...
   * will wither compare the result of the query or generate new output files.
   * <p>As the comparison is exact one make sure you do not have any blank nodes in the result!
   *
   ...
   * on the LEARN_MODE setting this method will wither compare the result of the query or generate new output files.
   *
   * @param query query to be executed
   * @param resultFile file containing the output to be compared against.
   */
  ...
  protected void verifyResult(String query, File resultFile, boolean ordered) throws Exception {
    try (RepositoryConnection conn = getRepository().getConnection()) {
      ...
      try (OutputStream os = new FileOutputStream(writeTo)) {
        while (iter.hasNext()) {
          String bs = iter.next().toString().replace("^^<http://www.w3.org/2001/XMLSchema#string>", "");
          os.write(bs.getBytes(UTF_8));
          os.write("\n".getBytes(UTF_8));
          if (isLearnMode()) {
      ...
      List<String> exp = Files.readAllLines(resultFile.toPath(), UTF_8);
      List<String> act = Files.readAllLines(actualFile.toPath(), UTF_8);
  ...
  protected void loadFilesToMongo(File inputFolder) {
    ...
    for (File file : inputFolder.listFiles()) {
      ...
      collectDocumentContents(batch, file);
    }

    collection.insertMany(batch);
  }

  private void collectDocumentContents(List<Document> batch, File file) throws AssertionError {
    try {
      ...
      throw new AssertionError("Failed to load file content: " + file.getPath(), e);
    }
  }
   ...
   * actual comparing.
   *
   * <p>When running the actual tests this should always return false and can be set to true only when writing new test
   * cases.
   *
   * @return whether the learn mode is enabled or not
   */
  ...
  protected void addMongoDates() {
    for (Document doc : collection.find()) {
      ...
      for (Entry<String, Object> currentValue : graph.get(0).entrySet()) {
        Object object = currentValue.getValue();

        String key = currentValue.getKey();
        if (object instanceof Document document) {
          addMongoDates(document, id, key);
        } else if (object instanceof List list) {
          addMongoDates(list, id, key);
        }
      }
    }
  }
  ...
  @SuppressWarnings("unchecked")
  private void addMongoDates(List<Object> objects, ObjectId objectId, String prefix) {
    for (Object object : objects) {
      if (!(object instanceof Document document)) {
        continue;
      }

      for (Entry<String, Object> currentValue : document.entrySet()) {
        StringBuilder sb = new StringBuilder(prefix);
        if (currentValue.getValue() instanceof Document subDoc) {
          sb.append("[").append(currentValue.getKey());
          addMongoDates(subDoc, objectId, sb.toString());
        } else if (currentValue.getValue() instanceof List list) {
          sb.append("[" + currentValue.getKey().toString());
          addMongoDates(list, objectId, sb.toString());
        }
      }
    }
  }

  private void addMongoDates(Document doc, ObjectId objectId, String prefix) {
    if (doc.get("@type") == null || !"xsd:dateTime".equals(doc.get("@type"))) {
      return;
    }

    StringBuilder sb = new StringBuilder("@graph.0.");
    ...
    String dateVal = doc.get("@value").toString();
    try {
      Bson newValue = new Document(sb.toString(), dateFormat.parse(dateVal));
      collection.updateOne(new Document("_id", objectId), new Document("$set", newValue));
    } catch (ParseException exc) {
      throw new AssertionError("Failed to parse date value: " + dateVal);
    }
  }
  ...
  protected void connectToMongoDB() {
    try (RepositoryConnection conn = getRepository().getConnection()) {
      String query = CONNECT_MONGODB_LOCALHOST.formatted(getServiceName(), MONGODB_DATABASE, MONGODB_COLLECTION);
  ...
  protected void disconnectFromMongoDB() {
    try (RepositoryConnection conn = getRepository().getConnection()) {
      conn.prepareUpdate(DROP_MONGODB_LOCALHOST.formatted(getServiceName())).execute();
    }
  }
